...
export function HeroSection() {
  ...
  // The "Đăng Ký Ngay" CTA only makes sense for anonymous visitors —
  // hide it once we know the visitor is signed in. While the auth
  // probe is still loading we hide it too, matching the header
  // behaviour, so an already-authenticated user doesn't see a flash
  // of the register button on hard reloads.
  const { user, loading } = useAuth();
  const showRegisterCta = !loading && !user;
