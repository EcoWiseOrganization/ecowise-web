  ...
  if (factors.length === 0) {
    return (
      <div className="p-16 bg-white rounded-3xl border border-[#B8D6B0] flex items-center justify-center">
        <p className="text-[#AAAAAA] text-sm">{t("admin.ef.table.empty")}</p>
      </div>
    );
  }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#DAEDD5] bg-[#f9fdf7]">
              <th className="text-left px-5 py-3 text-[#6E726E] font-semibold text-xs uppercase tracking-wide">{t("admin.ef.table.name")}</th>
              <th className="text-left px-4 py-3 text-[#6E726E] font-semibold text-xs uppercase tracking-wide">{t("admin.ef.table.scope")}</th>
              <th className="text-left px-4 py-3 text-[#6E726E] font-semibold text-xs uppercase tracking-wide">{t("admin.ef.table.category")}</th>
              <th className="text-right px-4 py-3 text-[#6E726E] font-semibold text-xs uppercase tracking-wide">{t("admin.ef.table.co2eTotal")}</th>
              <th className="text-left px-4 py-3 text-[#6E726E] font-semibold text-xs uppercase tracking-wide">{t("admin.ef.table.unit")}</th>
              <th className="text-left px-4 py-3 text-[#6E726E] font-semibold text-xs uppercase tracking-wide">{t("admin.ef.table.source")}</th>
              <th className="text-center px-4 py-3 text-[#6E726E] font-semibold text-xs uppercase tracking-wide">{t("admin.ef.table.year")}</th>
              <th className="text-right px-5 py-3 text-[#6E726E] font-semibold text-xs uppercase tracking-wide">{t("admin.ef.table.actions")}</th>
            </tr>
          </thead>
              ...
              <tr key={ef.id} className="hover:bg-[#fafffe] transition-colors">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-2">
                    <button
                      onClick={() => onEdit(ef)}
                      title={t("admin.ef.table.actions")}
