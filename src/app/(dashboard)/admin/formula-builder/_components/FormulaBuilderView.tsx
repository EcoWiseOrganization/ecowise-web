...
export function FormulaBuilderView({ categories, factors, initialTemplates }: FormulaBuilderViewProps) {
  ...
  const handleCreate = async (input: CreateCalculationTemplateInput) => {
    ...
    // Build joined view for immediate display
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between">
        <p className="text-[#AAAAAA] text-sm">{templates.length} template{templates.length !== 1 ? "s" : ""} defined</p>
        <button
      ...
      {templates.length === 0 ? (
        <div className="py-20 border-2 border-dashed border-[#DAEDD5] rounded-2xl flex flex-col items-center gap-3">
          ...
          <p className="text-[#3B3D3B] text-sm font-medium">No templates yet</p>
          <p className="text-[#AAAAAA] text-xs">Create the first calculation template to get started.</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                {tmpl.default_ef && (
                  <div className="shrink-0 text-right">
                    <p className="text-[#AAAAAA] text-xs">Default EF</p>
