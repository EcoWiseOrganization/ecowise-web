...
function StatusBadge({ status }: { status: string }) {
  return (
    <span
      className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${STATUS_STYLES[status] ?? "bg-gray-100 text-gray-600 border-gray-200"}`}
    >
      {status}
    </span>
  );
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white rounded-2xl border border-[#DAEDD5] px-6 py-5 flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="flex-1 min-w-0">
          <h1 className="text-[#141514] text-xl font-bold truncate">{org.name}</h1>
      ...
      {/* Members section */}
      <section>
        <h2 className="text-[#141514] text-base font-semibold mb-3">
              ...
              return (
                ...
                >
                  ...
                  {m.role === "Organization Admin" && (
                    <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-[#f0f9ed] text-[#1F8505] border border-[#DAEDD5]">
                      Admin
                    </span>
                  )}
                </div>
            ...
            <div className="text-center">
              ...
              {isAdmin && (
                <p className="text-[#AAAAAA] text-xs mt-1">
                  Create your first event for this organization.
                </p>
              )}
            </div>
            {isAdmin && (
