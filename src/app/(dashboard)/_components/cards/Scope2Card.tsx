...
/**
 * Scope-2 card for the personal dashboard. The original mock split
 * the value into Location-based vs Market-based; we don't track that
 * distinction per personal log yet, so we surface a single number
 * and the % share like the other scope cards.
 */
export function Scope2Card({ scope2Kg, totalKg }: Props) {
  const { t } = useTranslation();
  const tonnes = scope2Kg / 1000;
  const showInKg = tonnes < 0.1 && scope2Kg > 0;
  ...
  const percentage = totalKg > 0 ? Math.round((scope2Kg / totalKg) * 100) : 0;
