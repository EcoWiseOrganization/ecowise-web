  ...
  return (
    <div className="bg-white shadow-[0px_1px_2px_rgba(0,0,0,0.05)] overflow-hidden rounded-3xl border border-[#B8D6B0] flex flex-col min-w-0">
      ...
      <div className="px-4 sm:px-6 py-4 sm:py-6 border-b border-[#B8D6B0] flex items-center justify-between gap-2">
        ...
        <button className="shrink-0 text-[#155A03] text-xs font-bold leading-4 bg-transparent border-none cursor-pointer hover:underline">
          {t("dashboard.recentEntries.viewAll")}
        </button>
      </div>

      {/* Table — horizontally scrollable on narrow screens to preserve column layout */}
      <div className="w-full overflow-x-auto">
        <div className="min-w-[640px]">
          {/* Table Header */}
          <div className="grid grid-cols-[1fr_0.8fr_0.6fr_0.7fr] bg-[#DAEDD5]">
            {RECENT_ENTRIES.columns.map((col) => (
              <div key={col} className="px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-center">
