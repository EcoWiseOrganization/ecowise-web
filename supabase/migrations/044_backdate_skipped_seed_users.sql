-- ================================================================
-- EcoWise: Backdate các user có sẵn trùng với danh sách seed 043
--
--   • Email nào trong danh sách 043 đã tồn tại từ trước (bị skip)
--     sẽ được backdate created_at về đúng lịch 10/06 -> 03/07/2026
--     ở cả public."User", auth.users và auth.identities.
--   • Row nào 043 vừa seed thì created_at đã khớp -> tự bỏ qua
--     (WHERE created_at IS DISTINCT FROM ...).
--   • full_name chỉ bị ghi đè nếu đang trống hoặc là auto-generated
--     (trùng phần trước @ của email); tên thật người dùng tự đặt
--     được giữ nguyên. avatar_url chỉ set khi đang NULL.
--   • Tắt tạm trg_audit_user như 043 (AuditLogs immutable).
...
BEGIN
  ...
  LOOP
    SELECT id INTO uid FROM public."User" pu WHERE lower(pu.email) = u.email;
    IF uid IS NULL THEN
      CONTINUE;  -- không có trong hệ thống (không thuộc diện backdate)
    END IF;

    -- 1) public."User" ---------------------------------------------------
    UPDATE public."User" pu SET
      created_at    = u.created_at,
      full_name     = CASE
                        WHEN pu.full_name IS NULL
                          OR btrim(pu.full_name) = ''
                          OR lower(pu.full_name) = u.user_name
                        THEN u.full_name
                        ELSE pu.full_name
                      END,
      user_name     = COALESCE(NULLIF(btrim(pu.user_name), ''), u.user_name),
      avatar_url    = COALESCE(pu.avatar_url, u.avatar_url),
      last_login_at = CASE
                        WHEN pu.last_login_at IS NULL
                          OR pu.last_login_at < u.created_at
                        THEN u.last_login_at
                        ELSE pu.last_login_at
                      END
    WHERE pu.id = uid
      AND pu.created_at IS DISTINCT FROM u.created_at;

    IF NOT FOUND THEN
      CONTINUE;  -- created_at đã khớp (row do 043 seed) -> bỏ qua
    END IF;

    n := n + 1;
    RAISE NOTICE 'backdated: %', u.email;

    -- 2) auth.users (nếu có tài khoản auth) -------------------------------
    UPDATE auth.users au SET
      created_at           = u.created_at,
      email_confirmed_at   = u.created_at,
      confirmation_sent_at = u.created_at - INTERVAL '2 minutes',
      last_sign_in_at      = CASE
                               WHEN au.last_sign_in_at IS NULL
                                 OR au.last_sign_in_at < u.created_at
                               THEN u.last_login_at
                               ELSE au.last_sign_in_at
                             END,
      updated_at           = GREATEST(COALESCE(au.last_sign_in_at, u.last_login_at), u.last_login_at)
    WHERE au.id = uid;
    ...
    UPDATE auth.identities ai SET
      ...
      last_sign_in_at = CASE
                          WHEN ai.last_sign_in_at IS NULL
                            OR ai.last_sign_in_at < u.created_at
                          THEN u.created_at
                          ELSE ai.last_sign_in_at
                        END
    WHERE ai.user_id = uid;
  END LOOP;

  RAISE NOTICE 'Backdated % existing users.', n;
END $$;

ALTER TABLE public."User" ENABLE TRIGGER trg_audit_user;

-- Kiểm tra sau khi chạy: tất cả email trong danh sách phải nằm trong khoảng
-- SELECT email, full_name, created_at FROM public."User"
--   WHERE created_at >= '2026-06-10' AND created_at < '2026-07-04'
--   ORDER BY created_at;
